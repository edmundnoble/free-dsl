...
object WhatIsAProgram {
  ...
  object Why {
    ...
    def ofLoggingOp[A](loggingOp: LoggingOps[A]): LoggingAndStorageOps[A] = loggingOp.left
    def ofStorageOp[A](storageOp: StorageOps[A]): LoggingAndStorageOps[A] = storageOp.right

    def logValueAtKey(key: String): Program[LoggingAndStorageOps, Unit] = for {
      storedValue <- inject(ofStorageOp(Storage.Get(key)))
      _ <- inject(ofLoggingOp(Logging.Info(() => s"value at $key: ${storedValue.toString}")))
    } yield ()
